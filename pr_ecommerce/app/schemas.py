...
class Product(BaseModel):
    ...
    rating: float = Field(..., ge=0, le=5, description='Оценка отзыва')

    model_config = ConfigDict(from_attributes=True)

class UserCreate(BaseModel):
    email: EmailStr = Field(description='Email пользователя')
    password: str = Field(min_length=8, description='Пароль (минимум  символов')
...
class ReviewCreate(BaseModel):
    product_id: int
    comment: str|None = Field(None, description='Содержние отзыва')
    grade: int = Field(ge=1, le=5, description='Оценка отзыва')
    model_config = ConfigDict(from_attributes=True)
